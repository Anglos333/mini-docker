...
`mini-docker` 是一个基于 **Go 语言和 Linux 内核底层特性** 从零实现的轻量级沙箱容器运行时。项目摒弃了繁琐的工业包装，用高内聚、低耦合的模块化架构，完整闭环了现代容器底层的核心隔离、硬资源配额、分层文件系统及网络自适应编排链路，旨在直观揭示现代容器引擎（Docker/runc）的核心黑科技。
